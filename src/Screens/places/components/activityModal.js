...
export default class ActivityModal extends Component {
  ...
  render() {
    const { selectedActivity, modalVisible, setModalVisible, addToCartAndHide, displayError } = this.props;
    const { startDate, endDate, noOfPeople } = this.state;
    const uri = selectedActivity && selectedActivity.image ? `${serverUrl}/images/activities/${selectedActivity.image}` : `${serverUrl}/images/default.png`;
